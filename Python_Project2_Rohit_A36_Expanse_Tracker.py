...
def submitexpense():
    values=[dateEntry.get(),Title.get(),Expense.get()]
    print(values)
    ...
    (?, ?, ?)
    '''
    curr.execute(query,(dateEntry.get(),Title.get(),Expense.get()))
    connectionObjn.commit()
...
def viewexpense():
    ...
    l=Label(root,text="Date\t  Title\t  Expense",font=('arial',15,'bold'),bg="DodgerBlue2",fg="white")
    ...
    m = Label(root, text = f"Total: {total_amount}", font=('arial',16))
